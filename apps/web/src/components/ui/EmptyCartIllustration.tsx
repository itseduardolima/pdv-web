...
// Ilustração de carrinho vazio. Só o carrinho segue a cor primária do tenant
// (contorno mais escuro, aro/alça na cor, trançado da cesta mais claro); o
// resto é neutro. Mesmo padrão de EmptyBoxIllustration.
const cartStyle = {
  '--empty-cart-base': 'var(--color-primary)',
  '--empty-cart-shade': 'color-mix(in srgb, var(--color-primary) 78%, black)',
  '--empty-cart-tint': 'color-mix(in srgb, var(--color-primary) 55%, white)',
} as CSSProperties

export function EmptyCartIllustration({ className = '' }: { className?: string }) {
  return (
    <svg viewBox="0 0 650 512" aria-hidden="true" className={className} style={cartStyle}>
      ...
      />
      <circle cx="340.677" cy="148.55" r="46.959" fill="var(--empty-cart-shade)" />
      <path
        fill="none"
        stroke="var(--empty-cart-tint)"
        strokeLinecap="round"
      ...
      />
      <circle cx="298.105" cy="428.058" r="18.743" fill="var(--empty-cart-base)" />
      <circle cx="298.105" cy="428.058" r="8.651" fill="#dbe8ec" />
      <circle cx="406.224" cy="428.058" r="18.743" fill="var(--empty-cart-base)" />
      ...
      />
      <rect width="39.6" height="18.36" x="103.858" y="130.248" fill="var(--empty-cart-base)" rx="2" />
      <circle cx="340.677" cy="179.6" r="2.7" fill="var(--empty-cart-tint)" />
    </svg>
  )
}
