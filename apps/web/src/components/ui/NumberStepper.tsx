...
export function NumberStepper({ label, value, onChange, error, min = 0 }: NumberStepperProps) {
  ...
  const button = 'flex h-7 w-7 shrink-0 items-center justify-center rounded-pill border-[1.5px] border-ink bg-surface font-body text-base leading-none text-ink'
  return (
    <div className="flex flex-col gap-1.5">
      ...
      >
        <button type="button" aria-label="Diminuir" onClick={() => onChange(Math.max(min, value - 1))} className={button}>
          −
        </button>
