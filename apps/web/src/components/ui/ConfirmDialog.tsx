    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-30 flex w-[calc(100%-32px)] max-w-md -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-card bg-surface p-6 md:p-8">
          ...
          {description && (
            <Dialog.Description className="font-body text-sm text-ink/60">{description}</Dialog.Description>
          )}
          <div className="mt-2 flex flex-col gap-2 sm:flex-row-reverse">
            <Button onClick={onConfirm} state={confirmState} className={destructive ? '!bg-danger !text-surface' : ''}>
              {confirmLabel}
            </Button>
