...
- [ ] 9.6 — Monitor de uptime externo (UptimeRobot ou equivalente) — **P0, bloqueia produção**: depende de 9.5 existir primeiro. Hostinger não tem isso pronto no hPanel (só monitora uso de recursos da própria VPS) — precisa de conta em serviço externo, e criar conta em serviço de terceiro é ação que sempre fica pro usuário (nunca é feita por um agente). Passo a passo (2026-09-17):
  1. Criar conta grátis em [uptimerobot.com](https://uptimerobot.com)
  2. "Add New Monitor" → tipo **HTTP(s)**
  3. URL: `https://api.bananapdv.com.br/health`
  4. Intervalo: 5 min (plano grátis)
  5. Alerta por e-mail (padrão da conta)
  6. Marcar este item `[x]` quando estiver configurado
